...
The seam between the two is `crates/scylla-extension`, the action pipeline. Every write in the core is a command (`CreateProject { organization_id, name, description }`) that `Actions::run` moves through three typed stages: `Authorize` (the permission check, run by the core for every action), `Prepare` (the use case reads and builds a staged value, it does not write) and `Persist` (the store writes inside `Prepared::commit`). Every read is a query (`GetProject { id }`) that the same `Actions::run` moves through `Authorize` and `Fetch`; the trait the action implements (`Command` or `Query`) decides what follows the permission check. Around each stage the `Hooks` registry runs six positions, in this order:
