...
The seam between the two is `crates/scylla-extension`, the action pipeline. Every write in the core is a command (`CreateProject { organization_id, name, description }`) that `Actions::send` moves through three typed stages: `Authorize` (the permission check, run by the core for every action), `Prepare` (the use case reads and builds a staged value, it does not write) and `Persist` (the store writes inside `Prepared::commit`). Every read is a query (`GetProject { id }`) that `Actions::query` moves through `Authorize` and `Fetch`. Around each stage the `Hooks` registry runs six positions, in this order:
...
The Community Edition registers nothing: the pipeline runs the permission check, the stages and no hook. A use case is a struct that holds its ports and implements `Run<Prepare<C>>` and `Run<Persist<C>>` per command (`commands.rs`, `prepare.rs`, `persist.rs`) and `Run<Fetch<Q>>` per query (`queries.rs`, `fetch.rs`); it has no method of its own. The gRPC handler is the adapter: it builds the command or the query from the request and calls `actions.send(&*use_cases, &caller, command)` or `actions.query(...)`. The project use case and its handler are the reference; see `crates/scylla-extension/AGENTS.md` for the positions, the decision table and the failure semantics.
