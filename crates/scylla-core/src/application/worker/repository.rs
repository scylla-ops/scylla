...
/// Aggregate run statistics for a single worker, derived from the `jobs` it
...
/// Persistence for the `workers` table — the 1:1 specialization of an app that
/// runs jobs. A row exists only for apps that are workers. Stats are derived
/// from the `jobs` table (`worker_app_id`), not stored here.
#[async_trait]
