...
impl UserOrganizationRepository for SurrealUserOrganizationRepository {
    ...
    async fn add_member(&self, user_id: &UserId, org_id: &OrganizationId) -> DomainResult<()> {
        ...
        self.db
            .query("RELATE $user_id -> user_organization -> $org_id")
            .bind(("user_id", user_record))
            .bind(("org_id", org_record))
            .await
    ...
    async fn remove_member(&self, user_id: &UserId, org_id: &OrganizationId) -> DomainResult<()> {
        ...
        self.db
            .query("DELETE FROM user_organization WHERE in = $user_id AND out = $org_id")
            .bind(("user_id", user_record))
            .bind(("org_id", org_record))
            .await
            .map_err(|e| DomainError::infrastructure(format!("Database error: {e}")))?;
    ...
    async fn is_member(&self, user_id: &UserId, org_id: &OrganizationId) -> DomainResult<bool> {
        ...
        let count_result: Vec<i64> = self
            .db
            .query("SELECT count() FROM user_organization WHERE in = $user_id AND out = $org_id GROUP ALL")
            .bind(("user_id", user_record))
            .bind(("org_id", org_record))
            .await
            .map_err(|e| DomainError::infrastructure(format!("Database error: {e}")))?
    ...
    async fn list_members(
        ...
        let count_result: Vec<i64> = self
            .db
            .query("SELECT count() FROM user_organization WHERE out = $org_id GROUP ALL")
            .bind(("org_id", org_record.clone()))
            .await
            .map_err(|e| DomainError::infrastructure(format!("Database error: {e}")))?
            .take("count")
            .map_err(|e| DomainError::infrastructure(format!("Query error: {e}")))?;
        ...
        let results: Vec<UserId> = self
            .db
            .query("SELECT VALUE in FROM user_organization WHERE out = $org_id LIMIT $limit START $start")
            .bind(("org_id", org_record))
    ...
    async fn list_user_organizations(
        ...
        let count_result: Vec<i64> = self
            .db
            .query("SELECT count() FROM user_organization WHERE in = $user_id GROUP ALL")
        ...
        let results: Vec<OrganizationId> = self
            .db
            .query("SELECT VALUE out FROM user_organization WHERE in = $user_id LIMIT $limit START $start")
            .bind(("user_id", user_record))
            .bind(("limit", params.limit()))
            .bind(("start", params.offset()))
            .await
...
mod tests {
    ...
    async fn setup() -> Surreal<Any> {
        ...
        db.query("DEFINE TABLE IF NOT EXISTS user_organization TYPE RELATION IN users OUT organizations SCHEMALESS")
            .await
            .unwrap()
            .check()
            .unwrap();
    ...
    async fn test_add_member() {
        ...
        repo.add_member(&user_id, &org_id)
            .await
    ...
    async fn test_remove_member() {
        ...
        repo.add_member(&user_id, &org_id).await.unwrap();

        repo.remove_member(&user_id, &org_id)
            .await
            .expect("Failed to remove member");

        let is_member = repo.is_member(&user_id, &org_id).await.unwrap();
        assert!(!is_member);
    }
    ...
    async fn test_remove_nonexistent_member() {
        ...
        let result = repo.remove_member(&test_user_id(), &test_org_id()).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_is_member_false() {
        ...
        let is_member = repo
            .is_member(&test_user_id(), &test_org_id())
            .await
            .unwrap();
        assert!(!is_member);
    }
    ...
    async fn test_list_members_empty() {
        ...
        let result = repo.list_members(&org_id, Some(&pagination)).await.unwrap();
    ...
    async fn test_list_members() {
        ...
        repo.add_member(&user1, &org_id).await.unwrap();
        repo.add_member(&user2, &org_id).await.unwrap();
        repo.add_member(&test_user_id(), &other_org).await.unwrap();
    ...
    async fn test_list_user_organizations() {
        ...
        repo.add_member(&user_id, &org1).await.unwrap();
        repo.add_member(&user_id, &org2).await.unwrap();
        repo.add_member(&other_user, &test_org_id()).await.unwrap();
