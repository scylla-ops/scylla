...
#[cfg(feature = "rest")]
pub async fn start_rest(config: &CoreConfig, services: &Services) -> Result<()> {
    use scylla_api::rest::{self, AppState, openapi::ApiDoc};
    use tower_http::trace::TraceLayer;
    use utoipa::OpenApi;
    use utoipa_swagger_ui::SwaggerUi;

    let state = AppState {
        auth_uc: services.auth_uc.clone(),
        user_uc: services.user_uc.clone(),
        org_uc: services.org_uc.clone(),
        project_uc: services.project_uc.clone(),
        pipeline_uc: services.pipeline_uc.clone(),
        job_uc: services.job_uc.clone(),
        permission_uc: services.permission_uc.clone(),
        permission_checker: services.permission_checker.clone(),
        session_repo: services.session_repo.clone(),
    };
