...
/// Mapper between Job domain entity and database records
pub struct JobMapper;

impl JobMapper {
    /// Convert database record to domain entity
    pub fn to_domain(record: JobRecord) -> DomainResult<Job> {
    ...
    pub fn to_insert(job: &Job) -> JobInsert {
        JobInsert {
    ...
    pub fn to_update(job: &Job) -> JobUpdate {
        JobUpdate {
            status: job.status().as_str().to_string(),
        }
    }
