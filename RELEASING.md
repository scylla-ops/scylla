...
- [ ] `git submodule status` shows no `+` or `-` prefix, and the pinned
      `scylla-protos` commit is pushed: the build context needs the protos,
      and nobody else can build a tag whose pin is not on GitHub
